...
{
    public partial class StateEquipmentUse_frm : Form
    {
        int _patientID = 0, _visitCode = 0;
        string _status;
        int PreNextVisitCode = 0;

        public StateEquipmentUse_frm(int PatientID, int VisitCode, string Status)
        {
            InitializeComponent();
        ...
        DataClassesSecondDataContext DCSSSS = new DataClassesSecondDataContext(Global_Cls.ConnectionStr);

        #region Load
        private void StateEquipmentUse_frm_Load(object sender, EventArgs e)
        {
        ...
        private void FillComponents(int VisitCodeSet)
        {
            try
            {
                ...
                foreach (short fg in (from g in DCSSSS.TBL_VisitStateEquipmentUses where g.PatientID == _patientID && g.VisitCode == VisitCodeSet select g.EquipmentUseId))
                {
                    UserControls.StateEquipmentUse_UC SSUC = new UserControls.StateEquipmentUse_UC(_patientID, VisitCodeSet, fg);
                    SSUC.Tag = fg;
        ...
        private void buttonItem_OK_Click(object sender, EventArgs e)
        {
            if (Global_Cls.Message_MehrGostar(0, Global_Cls.MessageType.SConfirmation, true, "آیا مایلید تغییرات در ویزیت مورخ " + textBoxVisitDate.Text + " اعمال شود؟"))
            {
                ...
                {
                    (fg as UserControls.StateEquipmentUse_UC).save(_patientID, PreNextVisitCode, short.Parse(fg.Tag.ToString()), fg.Visible);
                    if (!(fg as UserControls.StateEquipmentUse_UC).SaveResult)
                    {
                        Global_Cls.Message_MehrGostar(0, Global_Cls.MessageType.SError, false, (fg as UserControls.StateEquipmentUse_UC).MessageResultStateEquipmentUse);
                        return;
                    }
                }
                //DCSSSS.SP_SetStateEquipmentUseIdInVisitStateEquipmentUse(_patientID, PreNextVisitCode);
        ...
        private void buttonAddEquipmentUse_Click(object sender, EventArgs e)
        {
            short fg = 0;
            UserControls.StateEquipmentUse_UC SSUC = new UserControls.StateEquipmentUse_UC(_patientID, PreNextVisitCode);//, ref fg);
            SSUC.Tag = fg;
            SSUC.Controls.Owner.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
            tableLayoutPanelVisit.Controls.Add(SSUC, 1, i++);
        }

        private void StateEquipmentUse_frm_FormClosed(object sender, FormClosedEventArgs e)
        {
            TBL_PatientVisit g = DCMD.TBL_PatientVisits.First(h => h.PatientID == _patientID && h.VisitCode == _visitCode);
            g.EquipmentUseDsc = DCSSSS.VisitStateEquipmentUseStr(_patientID, _visitCode);
            DCMD.SubmitChanges();
        }
