...
{
    public partial class StateBedsore_frm : Form
    {
        int _patientID = 0, _visitCode = 0;
        string _status;
        int PreNextVisitCode = 0;

        public StateBedsore_frm(int PatientID, int VisitCode, string Status)
        {
            InitializeComponent();
        ...
        #region Load
        private void StateBedsore_frm_Load(object sender, EventArgs e)
        {
        ...
        private void FillComponents(int VisitCodeSet)
        {
            try
            {
                ...
                foreach (short fg in (from g in DCMD.TBL_VisitStateBedsores where g.PatientID == _patientID && g.VisitCode == VisitCodeSet select g.StateBedsoreId))
                {
                    UserControls.StateBedsore_UC SSUC = new UserControls.StateBedsore_UC(_patientID, VisitCodeSet, fg);
                    SSUC.Tag = fg;
        ...
        private void buttonItem_OK_Click(object sender, EventArgs e)
        {
            if (Global_Cls.Message_MehrGostar(0, Global_Cls.MessageType.SConfirmation, true, "آیا مایلید تغییرات در ویزیت مورخ " + textBoxVisitDate.Text + " اعمال شود؟"))
            {
                ...
                {
                    (fg as UserControls.StateBedsore_UC).save(_patientID, PreNextVisitCode, short.Parse(fg.Tag.ToString()));
                    if (!(fg as UserControls.StateBedsore_UC).SaveResult)
                    {
                        Global_Cls.Message_MehrGostar(0, Global_Cls.MessageType.SError, false, (fg as UserControls.StateBedsore_UC).MessageResultStateBedsore);
                        return;
                    }
                }
                DCMD.SP_SetStateBedsoreIdInVisitStateBedsore(_patientID, PreNextVisitCode);
                Close();
            }
        }
        ...
        private void buttonAddBedsore_Click(object sender, EventArgs e)
        {
            short fg = 0;
            UserControls.StateBedsore_UC SSUC = new UserControls.StateBedsore_UC(_patientID, PreNextVisitCode, ref fg);
            SSUC.Tag = fg;
            SSUC.Controls.Owner.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
            tableLayoutPanelVisit.Controls.Add(SSUC, 1, i++);
        }

        private void StateBedsore_frm_FormClosed(object sender, FormClosedEventArgs e)
        {
            TBL_PatientVisit g = DCMD.TBL_PatientVisits.First(h => h.PatientID == _patientID && h.VisitCode == _visitCode);
            g.BedsoreDsc = DCMD.VisitStateBedsoreStr(_patientID, _visitCode);
            DCMD.SubmitChanges();
        }
