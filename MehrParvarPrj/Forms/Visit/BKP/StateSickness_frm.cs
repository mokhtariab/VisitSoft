...
{
    public partial class StateSickness_frm : Form
    {
        int _patientID = 0, _visitCode = 0;
        string _status;
        int PreNextVisitCode = 0;

        public StateSickness_frm(int PatientID, int VisitCode, string Status)
        {
            InitializeComponent();
       ...
       DataClasses_MainDataContext DCMD = new DataClasses_MainDataContext(Global_Cls.ConnectionStr);
        ...
        #region Load
        private void StateSickness_frm_Load(object sender, EventArgs e)
        {
        ...
        private void FillComponents(int VisitCodeSet)
        {
            try
            {
                ...
                foreach (short fg in (from g in DCMD.TBL_VisitStateSicknesses where g.PatientID == _patientID && g.VisitCode == VisitCodeSet select g.StateSicknessId))
                {
                    UserControls.StateSickness_UC SSUC = new UserControls.StateSickness_UC(_patientID, VisitCodeSet, fg);
                    SSUC.Tag = fg;
        ...
        private void buttonItem_OK_Click(object sender, EventArgs e)
        {
            if (Global_Cls.Message_MehrGostar(0, Global_Cls.MessageType.SConfirmation, true, "آیا مایلید تغییرات در ویزیت مورخ " + textBoxVisitDate.Text + "اعمال شود؟"))
            {
                ...
                {
                    (fg as UserControls.StateSickness_UC).save(_patientID, PreNextVisitCode, short.Parse(fg.Tag.ToString()));
                    if (!(fg as UserControls.StateSickness_UC).SaveResult)
                    {
                        Global_Cls.Message_MehrGostar(0, Global_Cls.MessageType.SError, false, (fg as UserControls.StateSickness_UC).MessageResultStateSickness);
                        return;
                    }
                }
               // DCMD.SP_SetStateSicknessIdInVisitStateSickness(_patientID, PreNextVisitCode);
                Close();
            }
        }
        ...
        private void buttonAddStateSickness_Click(object sender, EventArgs e)
        {
            short fg = 0;
            UserControls.StateSickness_UC SSUC = new UserControls.StateSickness_UC(_patientID, PreNextVisitCode, ref fg);
            SSUC.Tag = fg;
            SSUC.Controls.Owner.Anchor = AnchorStyles.Left | AnchorStyles.Right | AnchorStyles.Top;
            tableLayoutPanelVisit.Controls.Add(SSUC, 1, i++);
        }

        private void StateSickness_frm_FormClosed(object sender, FormClosedEventArgs e)
        {
            TBL_PatientVisit g = DCMD.TBL_PatientVisits.First(h => h.PatientID == _patientID && h.VisitCode == _visitCode);
            g.StateSicknessDsc = DCMD.VisitStateSicknessStr(_patientID, _visitCode);
            DCMD.SubmitChanges();
        }
